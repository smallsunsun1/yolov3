...
def blend(image1, image2, factor):
    ...
    difference = tf.cast(image2 - image1, tf.float32)
...
def random_shift_bbox(image, bbox, pixel_scaling, replace,
    ...
    minval_y = clip_y(
        min_y - tf.cast(pixel_scaling * tf.cast(bbox_height, tf.float32) / 2.0, tf.int32))
    maxval_y = clip_y(
        min_y + tf.cast(pixel_scaling * tf.cast(bbox_height, tf.float32) / 2.0, tf.int32))
    minval_x = clip_x(
        min_x - tf.cast(pixel_scaling * tf.cast(bbox_width, tf.float32) / 2.0, tf.int32))
    maxval_x = clip_x(
        min_x + tf.cast(pixel_scaling * tf.cast(bbox_width, tf.float32) / 2.0, tf.int32))
    ...
    new_bbox = tf.stack([
        tf.cast(new_min_y, tf.float32) / tf.cast(image_height, tf.float32),
...
def rotate_with_bboxes(image, bboxes, degrees, replace):
    ...
    # pylint:disable=g-long-lambda
    # pylint:enable=g-long-lambda
    bboxes = tf.map_fn(lambda bbox: _rotate_bbox(
        bbox, image_height, image_width, degrees), bboxes)
    return image, bboxes
...
def translate_bbox(image, bboxes, pixels, replace, shift_horizontal):
    ...
    # pylint:enable=g-long-lambda
    bboxes = tf.map_fn(lambda bbox: _shift_bbox(
        bbox, image_height, image_width, pixels, shift_horizontal), bboxes)
    return image, bboxes
...
def shear_with_bboxes(image, bboxes, level, replace, shear_horizontal):
    ...
    image_width = tf.shape(image)[1]
    # pylint:enable=g-long-lambda
    bboxes = tf.map_fn(lambda bbox: _shear_bbox(
        bbox, image_height, image_width, level, shear_horizontal), bboxes)
    return image, bboxes
...
def unwrap(image, replace):
    ...
    flattened_image = tf.where(
        tf.equal(tf.expand_dims(alpha_channel, axis=-1), 0),
...
def distort_image_with_autoaugment(image, bboxes, augmentation_name):
    ...
    """
    logging.info('Using autoaugmention policy: {}'.format(augmentation_name))
